#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""逐句孤立修复第二章序诵尾部 idx 66-79。

根因: 强制对齐模型对公式化短句(如 Nidānuddeso paṭhamo / dve aniyatā dhammā 等)
低置信度, 全局/整块 Viterbi 会在这些句上"抢跑"压缩时间。词数正确、整体单调,
故原校对未捕获, 表现为某句没播完且其后串联错位。

修法: 对每句单独取其全局位置附近窗口[global0-1.0, global1+4.0], 仅对齐该句自身词,
避免相互偷时间/过拉伸; 再用 v4 中点边界法链式拼接 66-79, 重切片。
左锚点用原始可靠的 idx 65 结束时间; 右端(idx 79)延伸到音频末尾。
"""
...
A, B = 66, 79
ANCHOR_L = 65
ORIG_RAW = os.path.join(AUDIO, "ch2_align_raw.bak_fix66_20260801_144743.json")  # 原始(未修复)对齐, 取可靠锚点
...
def main():
    ...
    Bd = raw["bounds"]
    orig_raw = json.load(open(ORIG_RAW, encoding="utf-8"))
    G1_L = orig_raw["bounds"][str(ANCHOR_L)][1]   # 可靠原始锚点
    D = raw["D"]
    ...
    for idx in range(A, B + 1):
        ...
        g0r, g1r = Bd[str(idx)][0], Bd[str(idx)][1]
        win0 = max(0.0, g0r - WIN_LO); win1 = min(D + 0.3, g1r + WIN_HI)
    ...
    # 备份 + 写回 raw
    BAK = os.path.join(AUDIO, "ch%d_align_raw.bak_persent_%s.json" % (CH, time.strftime("%Y%m%d_%H%M%S")))
    ...
    BAK2 = os.path.join(AUDIO, "sent_new_bak_persent_%s" % time.strftime("%Y%m%d_%H%M%S"))
    if has:
